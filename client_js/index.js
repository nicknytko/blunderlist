...
function format_time(date) {
    ...
    if (date.getHours() > 12) {
        return (
            (date.getHours() - 12).toString() + ":" + pad(date.getMinutes()) + " PM"
        );
    } else {
...
function reload_list(cb) {
    api_all(function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            items.push({
                title: data[i].title,
                date: data[i].due_date,
                id: data[i].id,
            });

            if (data[i].reserve && data[i].reserve_date) {
                items.push({
                    title: 'RSVP for "' + data[i].title + '"',
...
function add_dom_event(label, date, id, parent, show_time) {
    ...
    if (events[id].class_org && events[id].class_org.match(/cs/i)) {
        icon = "file-code";
    }
    ...
    $(parent).append(
        list_link +
            name +
            '"><div class="d-flex w-100 justify-content-between"><span><img src="icon/' +
            icon +
            '.svg"></img> ' +
            label +
            "</span><span>" +
...
function load_event_form() {
    if (cur_event != -1) {
        ...
        $("#task_form_reserve").prop(
            "checked",
            event.reserve == "true" || event.reserve === true,
        );
...
function save_event_form(cb) {
    if (cur_event != -1) {
        ...
        if (due_date.length > 0 && isNaN(Date.parse(due_date))) {
            valid_dates = false;
        ...
        if (reserve_date.length > 0 && isNaN(Date.parse(due_date))) {
            valid_dates = false;
        ...
        if (valid_dates) {
            ...
            api_update(cur_event, event, function (data) {
                $("#task_form_update_btn").removeClass("disabled");
...
$(document).ready(function () {
    ...
    $("#task_form_back_btn").click(function () {
        select_item(-1);
    });

    api_version(function (data) {
        $("#version").html("Git version: " + data.git_small);
    });
});
